...
def decodeResponse(response):
    decodedHeader = {
        ...
        #"body": int.from_bytes(response[1], "big")
        #"body": response[13].decode('utf-8')
    }
...
def decodeBody(response):
    body = response[13]
    #bytesResponse = int.to_bytes(response[13])
    #body = response.partition(bytesResponse)
    #decodedBody = body[2].decode('utf-8')
    return body
...
def main():
    ...
    #Send query and receive response using socket
    sendQuery(domain, ipaddr, clientSocket)
    response = receiveResponse(clientSocket)

    #Decoding response
    decodedResponse = decodeResponse(response)
    decodedBody = decodeBody(response)

    #Testing response (seeing how message is displayed)
    #Remove this line in final project
    ...
    while(not foundAnswer):
         #Write code here to iterate through additional queries until an answer is found in the response.
