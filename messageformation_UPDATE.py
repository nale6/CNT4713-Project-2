...
def extractNextDNSIP(response):
    ...
    return domainList, ipList, nsList

#Used for displaying final output
def displayResponse(ipList, nsList, domainList, header, currentIP):
...
def main():
    ...
    while not foundAnswer:
        ...
        if decodedResponse["answerCount"] > 0:
            ...
            domainList, ipList, domainList = extractNextDNSIP(response)
        ...
        displayResponse(ipList, nsList, domainList, decodedResponse, currentIP)
        print("------------------")
